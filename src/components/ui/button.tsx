...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    // WORM_V2: Direct CSS variable injection for official styling
    return (
      <Comp 
        className={cn(buttonVariants({ variant, size, className }))} 
        ref={ref} 
        style={{
          borderRadius: 'var(--dynamic-button-radius, 4px)',
          backgroundColor: variant === 'default' ? 'var(--dynamic-primary)' : undefined,
          fontFamily: 'var(--dynamic-font-primary)',
